...
class CallbackQuery extends require("../../../Base") {
  ...
  initCallbackConfig() {
    ...
    if (!this.dbName) this.dbName = process.env.DATABASE_NAME;
  ...
  createDatabaseCallback(databaseName = this.dbName) {
    const dbFn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("createDatabase-error", error);
      }
  ...
  createCollectionCallback(collectionName = this.collection) {
    const fn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("createCollection-error", error);
      }
  ...
  insertOneCallback(data = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("insertOne-error", error);
      }
      const dbs = db.db(this.db);
  ...
  insertManyCallback(data = [], collectionName = this.collection) {
    ...
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("insertMany-error", error);
      }
      const dbs = db.db(this.db);
  ...
  createManyCallback(data = [], collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("createMany-error", error);
      }
      const dbs = db.db(this.db);
  ...
  findOneCallback(query = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("findOne-error", error);
      }
      const dbs = db.db(this.db);
  ...
  firstCallback(query = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("first-error", error);
      }
      const dbs = db.db(this.db);
  ...
  findCallback(query = {}, projection = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("find-error", error);
      }
      const dbs = db.db(this.db);
  ...
  allCallback(query = {}, projection = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("all-error", error);
      }
  ...
  sortCallback(
    ...
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("sort-error", error);
      }
      const dbs = db.db(this.db);
  ...
  deleteOneCallback(query = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("deleteOne-error", error);
      }
      const dbs = db.db(this.db);
      const cfn = (err, res) => {
        if (err) {
  ...
  deleteManyCallback(query = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("deleteMany-error", error);
      }
      const dbs = db.db(this.db);
      const cfn = (err, res) => {
        if (err) {
  ...
  collectionDropCallback(dbName = this.db, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("collectionDrop-error", error);
      }
      const dbs = db.db(dbName);
      const cfn = (err, res) => {
        if (err) {
  ...
  updateOneCallback(query = {}, data = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("updateOne-error", error);
      }
      const dbs = db.db(this.db);
      const cfn = (err, res) => {
        if (err) {
  ...
  updateCallback(query = {}, data = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("update-error", error);
      }
      const dbs = db.db(this.db);
  ...
  updateManyCallback(query = {}, data = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("updateMany-error", error);
      }
      const dbs = db.db(this.db);
      const cfn = (err, res) => {
        if (err) {
  ...
  limitCallback(
    ...
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("limit-error", error);
      }
      const dbs = db.db(this.db);
  ...
  letfJoinCallback(
    ...
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("letfJoin-error", error);
      }
      const dbs = db.db(this.db);
  ...
  findByIdCallback(id, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("findById-error", error);
      }
      const dbs = db.db(this.db);
  ...
  findByEmailCallback(email, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("findByEmail-error", error);
      }
      const dbs = db.db(this.db);
  ...
  firstByEmailCallback(email, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("firstByEmail-error", error);
      }
      const dbs = db.db(this.db);
  ...
  firstByUsernameCallback(username, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("firstByUsername-error", error);
      }
      const dbs = db.db(this.db);
  ...
  firstByPhoneCallback(phone, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("firstByPhone-error", error);
      }
      const dbs = db.db(this.db);
  ...
  findByQueryCallback(
    ...
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("findByQuery-error", error);
      }
      const dbs = db.db(this.db);
  ...
  firstByQueryCallback(query = {}, collectionName = this.collection) {
    const dbfn = (error, db) => {
      if (error) {
        db.close();
        return this.emit("firstByQuery-error", error);
      }
   ...
   */
     firstByTokenCallback(token, collectionName = this.collection) {
      const dbfn = (error, db) => {
        if (error) {
          db.close();
          return this.emit("firstByTokenCallback-error", error);
        }
        const dbs = db.db(this.db);
